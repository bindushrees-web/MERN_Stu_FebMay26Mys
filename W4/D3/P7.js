...
function fetchPost(url) {
    ...
    .then(function(response) {
        if (!response.ok) {
            throw new Error("Http error and Status Code:" + response.status);
        }
...
existingPostBtn.addEventListener("click",function(){
    fetchPost("https://jsonplaceholder.typicode.com/posts/26");
});

missingPostBtn.addEventListener("click",function(){
    fetchPost("https://jsonplaceholder.typicode.com/posts/110");
});